...
const tracks = [
  {
    title: "Hackathon",
    description:
      "A 48-hours sprint where,student teams design and prototype real solutions to real problems",
    button: "Join",
    img: "/images/join.svg",
  ...
  },
  {
    ...
    description:
      "An intensive bootcamp for early stage founders that are preparing to take their MVPs’ to market.",
    button: "Apply",
  ...
  },
  {
    ...
    description:
      "Personalized guidance from experienced industry mentors to help refine your skills and strategy.",
    button: "Meet Mentors",
    img: "/images/meet.svg",
...
const OurSignatureTracks = () => {
  ...
  useGSAP(() => {
    ...
    // gsap.fromTo(".main", { opacity:0, xPercent:100}, { opacity:1, xPercent:0, duration:1, ease:"power1,inOut", delay:0.3});
  ...
  return (
    <section className="pb-30 overflow-hidden">
      ...
      <div className="px-[10%] w-full pt-[5%] flex justify-between items-center ">
        <section className="max-w-[50%] flex flex-col gap-7">
          <h1 id="animate" className="font-orbitron font-bold text-3xl ">
            {tracks[currentIndex].title}
          </h1>
          <p id="animate" className="font-geist font-normal text-2xl description">
            {tracks[currentIndex].description}
          </p>
          <div
            id="animate"
            className={`relative  cursor-pointer but ${
              currentIndex == 2 ? "max-w-[280px]" : "max-w-[140px]"
            }`}
          >
            ...
            <p
              className={`font-geist font-normal  ${
                currentIndex == 2
                  ? "text-[27px] left-[30%] translate-x-[-20%]"
                  : "text-3xl left-[50%] translate-x-[-50%]"
              }  absolute text-white text-left  top-[50%] translate-y-[-50%] `}
            >
              {tracks[currentIndex].button}
            </p>
          </div>
          ...
          <ul className="flex max-w-[260px] gap-[10px] items-center">
            {tracks.map((track, index) => (
              <li
                ...
                key={track.title}
                className={` w-full rounded-full h-2 cursor-pointer transition-all duration-300 ${
                  index == currentIndex ? "bg-[#007060]" : "bg-[#007060]/40"
                }`}
              ></li>
            ))}
          </ul>
        </section>
        <section id="animate" className="max-w-[50%] main">
          <img
            className="max-w-[470px] max-h-[365px]"
            src={tracks[currentIndex].main}
            alt=""
          />
        </section>
      </div>
